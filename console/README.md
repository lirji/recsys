...
| 模式           | 登录方式                                      | 网关侧开关(必须成对!)       |
| -------------- | --------------------------------------------- | --------------------------- |
| `oidc`(Docker 默认) | **Casdoor 统一登录**(确认租户后走授权码+PKCE) | `RECSYS_EDGE_CASDOOR=true`  |
| `legacy`(回滚) | 演示账号(admin/advertiser/user,密码=用户名)   | `RECSYS_EDGE_CASDOOR=false` |

```bash
# Vite dev 显式起 oidc 模式(需 Casdoor :8000 + casdoor 模式网关;dev 账号 radmin/Radmin@12345、rowner1/Rowner1@12345)
